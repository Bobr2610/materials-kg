...
try:
    ...
    try:
        from kg_engine.agent.langsmith_config import get_langsmith_config, get_openai_wrapper
        from kg_engine.agent.logging_config import _parse_bool
        from kg_engine.agent.provider_adapters import MistralWrapper, is_mistral_model
        from kg_engine.agent.utils import ensure_valid_answer
...
class LLMManager:
        ...
        if fetch_at_startup:
            try:
                ...
                if api_key:
                    ...
                    from kg_engine.agent.utils.openrouter_pricing import (
    ...
    def _get_api_key(
        ...
        from kg_engine.agent.key_resolution import get_provider_api_key
    ...
    def _initialize_openrouter_llm(
        ...
        try:
            ...
            from kg_engine.agent.openrouter_native_chat import create_openrouter_native_chat_model
                ...
                if use_tools and self.LLM_CONFIGS.get(provider_enum, {}).tool_support:
                    ...
                    if tools_list:
                        try:
                            ...
                            try:
                                from kg_engine.agent.token_budget import (
    ...
    def _get_configured_provider_and_model_index(
        ...
        try:
            from kg_engine.agent.agent_config import get_llm_settings
    ...
    async def load_mcp_tools_if_enabled(self) -> None:
        ...
        try:
            ...
            from kg_engine.agent.mcp_tools import is_mcp_enabled, preload_mcp_tools
    ...
    def get_tools(self) -> list[Any]:
        ...
        # TODO[materials-kg]: CMW platform tools not yet ported.
        # tools.tools, tools.attributes_tools, tools.applications_tools, tools.templates_tools
        # were from cmw-platform-agent and are not relevant to materials science KG.
        # Replace with materials-specific tool modules when created.

        try:
            from .mcp_tools import get_cached_mcp_tools, is_mcp_enabled, merge_tools
        except ImportError:
            from kg_engine.agent.mcp_tools import (
...
def reset_mcp_tools_state(manager: LLMManager | None = None) -> None:
    ...
    try:
        ...
        from kg_engine.agent.mcp_tools import reset_mcp_tools_cache
